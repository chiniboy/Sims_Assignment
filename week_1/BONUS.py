...
while running:
    ...
    for _ in range(SIMULATION_SUBSTEPS):
        ...
        # displacement[i, j] points from ball j to ball i.
        displacement = position_array[:, None, :] - position_array[None, :, :]
        distance = np.linalg.norm(displacement, axis=2)
        minimum_distance = 2.0 * PARTICLE_RADIUS

        collision_mask = (distance < minimum_distance) & (distance > 1e-7)
        collision_mask &= ~np.eye(NUM_PARTICLES, dtype=bool)
        ...
        np.divide(
            ...
            distance[..., None],
            out=normal,
            where=distance[..., None] > 1e-7,
        )
        overlap = np.where(collision_mask, minimum_distance - distance, 0.0)

        # Push overlapping balls apart. Both directions of each pair are
        # represented, so the summed correction is symmetric.
        position_array += 0.5 * np.sum(overlap[..., None] * normal, axis=1)

        relative_velocity = velocity_array[:, None, :] - velocity_array[None, :, :]
        velocity_along_normal = np.sum(relative_velocity * normal, axis=2)
        approaching = collision_mask & (velocity_along_normal < 0.0)
        impulse = np.where(
            approaching,
            -(1.0 + RESTITUTION) * velocity_along_normal / 2.0,
            0.0,
        )
        velocity_array += np.sum(impulse[..., None] * normal, axis=1)
